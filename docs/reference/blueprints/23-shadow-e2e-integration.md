...
> **任务目标**: 编写自动化集成测试脚本 `tests/shadow-e2e-test.py`。该脚本需读取现有参数，动态将其前缀修改为隔离的 `omnitest`，安全地在云端拉起影子环境资源组，验证内网 Private Endpoint 及 DNS 解析连通性后，自动清退销毁资源组以防费用泄露。
...
### 2.1 执行要求
1. 在编写完脚本后，赋予可执行权限：`chmod +x tests/shadow-e2e-test.py`。
