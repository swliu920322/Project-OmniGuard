...
The left side of the `/kinematic` page contains seven sliders. Here is what each one does, in plain language.
...
- **Note**: This parameter only affects the **Cloud-Only** mode calculation.

## 4. LLM Token Rate (Cloud-Only Mode)
...
## 5. Prompt Tokens (Cloud-Only Mode)
...
## 6. Completion Tokens (Cloud-Only Mode)
...
## 7. Edge Reaction Time (Edge Fallback Mode)

- **What it is**: How long the local sensor takes to detect danger and trigger braking.
- **Range**: 1 to 100 milliseconds.
- **Effect**: Faster sensors allow higher safe speeds.
- **Real-world analogy**: The reaction time of a professional race car driver vs. an elderly person.
- **Note**: This parameter only affects the **Edge Fallback** mode calculation. Default is 15ms, representing typical ultrasonic/LiDAR sensor response time.
...
- Edge mode is always faster than cloud mode because it bypasses network round-trip and LLM inference entirely.

---

# 参数指南

`/kinematic` 页面左侧有七个滑块。下面是每个滑块的通俗解释。
...
## 3. Network RTT（网络往返时间，Cloud-Only 模式）

- **含义**：信号从机器人到云端再回来的时间。
- **范围**：20 到 2000 毫秒。
- **影响**：RTT 越大，云端总延迟越大，`V_max` 越低。
- **生活类比**：视频通话卡顿时感受到的延迟。
- **注意**：此参数仅影响 **Cloud-Only** 模式的计算。

## 4. LLM Token Rate（LLM token 速率，Cloud-Only 模式）
...
## 5. Prompt Tokens（输入 token 数，Cloud-Only 模式）
...
## 6. Completion Tokens（输出 token 数，Cloud-Only 模式）
...
## 7. Edge Reaction Time（边缘反应时间，Edge Fallback 模式）

- **含义**：本地传感器发现危险到触发刹车的时间。
- **范围**：1 到 100 毫秒。
- **影响**：传感器越快，允许的安全速度越高。
- **生活类比**：职业赛车手和老年人的反应时间差异。
- **注意**：此参数仅影响 **Edge Fallback** 模式的计算。默认值为 15ms，代表典型超声波/激光雷达传感器的反应时间。
...
- Edge 模式总是比 Cloud 模式快，因为它完全绕过了网络往返和大模型推理。
